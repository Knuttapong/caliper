...
var Client = class extends BaseClient {
	...
	getTargetOrderer(request_orderer, channel_orderers, channel_name) {
		...
		} else if(channel_orderers && Array.isArray(channel_orderers)) { //BonuZz
			var index = Math.floor(Math.random() * channel_orderers.length);
			if(channel_orderers[index]){
				orderer = channel_orderers[index];				
			}
			else{
